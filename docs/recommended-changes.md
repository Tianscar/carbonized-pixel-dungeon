...
If you wish to set it to 1 anyway, the various constant variables toward the top of [ShatteredPixelDungeon.java](/core/src/main/java/com/ansdoship/carbonizedpixeldungeon/ShatteredPixelDungeon.java) are a good starting point for finding all of the cases where the game refers to the version code for compatibility purposes.

## Credits & Supporter button

If you are making your own mod, you will likely want to add yourself to the credits or change the current supporter link. Feel free to adjust these however you like, the relevant code is in [AboutScene.java](/core/src/main/java/com/ansdoship/carbonizedpixeldungeon/scenes/AboutScene.java) and [SupporterScene.java](/core/src/main/java/com/ansdoship/carbonizedpixeldungeon/scenes/SupporterScene.java). If you wish to disable the supporter link, simply comment out the line `add(btnSupport);` in [TitleScene.java](/core/src/main/java/com/ansdoship/carbonizedpixeldungeon/scenes/TitleScene.java).

The game also has a one-time nag window that appears when the player first defeats Goo. If you wish to edit this window, the code is in [WndSupportPrompt.java](/core/src/main/java/com/ansdoship/carbonizedpixeldungeon/windows/WndSupportPrompt.java). If you wish to disable it entirely, the triggering logic is in [SkeletonKey.java](/core/src/main/java/com/ansdoship/carbonizedpixeldungeon/items/keys/SkeletonKey.java).
...
To simply disable news entirely, comment out the line `add(btnNews);` in [TitleScene.java](/core/src/main/java/com/ansdoship/carbonizedpixeldungeon/scenes/TitleScene.java).
...
- In [Languages.java](/core/src/main/java/com/ansdoship/carbonizedpixeldungeon/messages/Languages.java) remove all of the enum constants except for ENGLISH.
...
- Finally remove the language picker by commenting out the line `add( langsTab );` in [WndSettings.java](/core/src/main/java/com/ansdoship/carbonizedpixeldungeon/windows/WndSettings.java)

...
import com.ansdoship.carbonizedpixeldungeon.actors.mobs.Pylon;
...
import com.ansdoship.carbonizedpixeldungeon.levels.painters.CavesPainter;
...
public class CavesBossLevel extends Level {
	...
	protected boolean build() {
		...
		for (int i= 14*width(); i < length(); i++) {
			if (map[i] == Terrain.EMPTY) {
				if (patch[i - 14*width()]){
					map[i] = Terrain.WATER;
				} else if (Random.Int(Dungeon.isChallenged(Challenges.Challenge.STRONGER_BOSSES) ? 4 : 8) == 0){
	...
	public void eliminatePylon(){
		...
		int finalPylonsRemaining = Dungeon.isChallenged(Challenges.Challenge.STRONGER_BOSSES) ? 1 : 2;

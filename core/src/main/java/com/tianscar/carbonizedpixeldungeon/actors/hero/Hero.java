...
import com.tianscar.carbonizedpixeldungeon.actors.buffs.AnkhInvulnerability;
import com.tianscar.carbonizedpixeldungeon.actors.buffs.Awareness;
import com.tianscar.carbonizedpixeldungeon.actors.buffs.Barkskin;
...
import com.tianscar.carbonizedpixeldungeon.actors.mobs.Monk;
...
import com.tianscar.carbonizedpixeldungeon.items.keys.Key;
import com.tianscar.carbonizedpixeldungeon.items.keys.SkeletonKey;
...
public class Hero extends Char {
	...
	public float hunger() {
		...
		if (isAlive()) {
			...
			hunger =  hungerBuff == null ? Hunger.STARVING : Hunger.STARVING - hungerBuff.hunger();
		}
		else hunger = HGRecorded;
		return Math.max(0, hunger);
	}
	...
	private static final String HGRECORDED  = "hgrecorded";
	...
	public static void preview( GamesInProgress.Info info, Bundle bundle ) {
		...
		info.hg = (int) bundle.getFloat( HGRECORDED );
	...
	public int drRoll() {
		...
		if (dualWielding.weaponNotNull())  {
			...
			if (belongings.weapon() != null && belongings.extra() != null) {
				...
				if (STR() < ((Weapon)belongings.extra()).STRReq()){
					wepDr -= (((Weapon)belongings.extra()).STRReq() - STR());
				}
			}
			...
			else if (belongings.extra() != null) {
				if (STR() < ((Weapon)belongings.extra()).STRReq()){
					wepDr -= 2*(((Weapon)belongings.extra()).STRReq() - STR());
				}
			}
			if (wepDr > 0) dr += wepDr;
		}
